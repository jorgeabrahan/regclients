...
if (userIdLS == null) { open('index.html', '_self') }

/* Guradar arreglo de clientes en el almacenamiento local */
export const saveClientsLS = () => { localStorage.setItem('Clients', JSON.stringify(clientsArr)) };
/* ------------------------------------------------------ */
...
export const updateLiveTotal = () => {
    ...
    document.getElementById('totalLive').innerText = formatter.format(clientsTotal);
    return clientsTotal;
}
/* ------------------------------------ */
