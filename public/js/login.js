...
import { fbApp, fbStore } from './modules/GenVars.mjs';
import { formatter } from './modules/GenVars.mjs';
...
const createClientsHtml = (clients) => {
    ...
    for (let client of clients) {
        clientsHtml.push(`<p><span class="bold">${client.name}</span> <span>HNL ${formatter.format(Number(client.total))}</span></p>`);
    }
        ...
        <div class="live__header">
            <div class="live__info">
                ...
                <div class="live__group">
                    ...
                    <p class="flex"><span class="bold">Total:</span> <span>HNL <span>${formatter.format(Number(liveData.bestBuyer.total))}</span></span></p>
                    <p class="flex"><span class="bold">Articulos:</span> <span>${liveData.bestBuyer.articles}</span></p>
                </div>
                <p class="live__total flex"><span>Total:</span> <span>HNL <span>${formatter.format(liveData.total)}</span></span></p>
            </div>
...
const loadContentFromFb = (userId) => {
    fbStore.collection('vendedoras').doc(userId).get().then(seller => {
        ...
        weekTotal.innerText = formatter.format(totalLives);
...
document.getElementById('livesDelete').addEventListener('click', () => {
    if (confirm('Seguro que deseas eliminar todo y empezar una nueva semana')) {
        fbStore.collection('vendedoras').doc(userIdLS).delete().then(() => {
            livesCnt.innerHTML = "";
