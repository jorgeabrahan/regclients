...
frmClientData.clientName.addEventListener("input", ({ target }) =>
  impFuncs.allowJustLetters(target)
);
frmClientData.price.addEventListener("input", ({ target }) => {
  if (isNaN(Number(target.value)) && target.value !== "-")
...
frmClientData.addEventListener("submit", (e) => {
  ...
  if (clientNameVal === "" || priceVal == 0) {
    showModal(
      "Error al agregar",
      "Asegurate de llenar todos los campos antes de continuar o introducir una cantidad correcta."
    );
    return;
  }

  //   console.log(frmClientData.amount.value);
