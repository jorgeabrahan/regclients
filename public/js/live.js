...
import { fbStore, date, formatter } from './modules/GenVars.mjs';
import { updateAndSave, saveClientsLS, showModal, clientsHtmlCnt, frmEdit, modalEdit, clientsArr, setClientsArr, userIdLS, clientsDataList, updateClientsAmount, updateLiveTotal } from './modules/ClientsDOM.mjs';
...
const clientName = frmClientData.clientName;
const price = frmClientData.price;
/* ----------------------------------------- */
...
const getPrevLives = () => {
    ...
    fbStore.collection('vendedoras').doc(userIdLS).get()
        .then(seller => {
            if (seller.exists) fbLives = seller.data();
        })
        .catch(err => { showModal('Error al leer base de datos', `Si sigues utilizando la pagina la informacion no se guardara en la base de datos.\n\nReporta este error: ${err}`) });
}
/* --------------------------------------------------- */
...
const checkForPrevClients = () => {
    ...
    if (localStorage.getItem('Edit') == 'on') {
        showModal('Modo edicion', 'Usted se encuentra en el modo de edicion, todos los cambios que realice se guardaran en el live que esta editando.\n\nImportante: ninguno de los cambios realizados en este live se guardaran hasta que no de click en el boton de guardar');
    ...
    if (clientsArr.length > 0) { 
        showModal('Almacenamiento local', 'Habian clientes en el almacenamiento local de una sesion previa.');
    ...
    const time = `${impFuncs.formatTime(date.getHours())}:${impFuncs.formatTime(date.getMinutes())}:${impFuncs.formatTime(date.getSeconds())}`;
    ...
    /* --------------------------------------- */
    const warn = 'Mientras no se agreguen clientes a la lista, cada vez que se recargue la pagina la hora de inicio y la fecha seran actualizadas.';
    showModal(`Informacion del live`, `Hora de inicio: ${time}\nFecha: ${currentDate}\n\n${warn}`);
}
...
document.getElementById('btnEditCancel').addEventListener('click', () => modalEdit.classList.add('d-none'));

/* Eventos para los inputs del formulario para editar un cliente */
frmEdit.nameE.addEventListener('input', ({ target }) => impFuncs.allowJustLetters(target));
frmEdit.articlesE.addEventListener('input', ({ target }) => impFuncs.allowJustNumbers(target));
...
frmEdit.addEventListener('submit', e => {
    ...
    if (editNameVal === '' || editArticlesVal <= 0 || editTotalVal <= 0) {
        showModal('Error al modificar', 'Asegurate de llenar todos los campos antes de continuar o de introducir una cantidad valida.');
        return;
    }

    //Se busca un cliente con el mismo nombre y diferente id
    let clientRepeated = clientsArr.find(element => element.name == editNameVal && element.clientId !== modalEdit.id);
    ...
    for (let client of clientsArr) {
        ...
        if (client.name !== editNameVal) {
            ...
            clientsDataList.appendChild(clientFunc.createOption(editNameVal, client.clientId));//Se agrega el nuevo nombre para autocompletar en la lista de clientes
        }
...
clientName.addEventListener('input', ({ target }) => impFuncs.allowJustLetters(target));
price.addEventListener('input', ({ target }) => {
    if (isNaN(Number(target.value)) && target.value !== '-') target.value = target.value.slice(0, target.value.length - 1);
});
...
frmClientData.addEventListener('submit', e => {
    ...
    const priceVal = Number(price.value.trim());

    if (clientNameVal === '' || priceVal == 0) {
        showModal('Error al agregar', 'Asegurate de llenar todos los campos antes de continuar o introducir una cantidad correcta.');
        return;
    }
    ...
    if (clientFromList) {
        ...
        clientFunc.removeClientFromArr(clientFunc.getClientIndexById(clientFromList.clientId)); //Se elimina del arreglo
        sortLive();
        return;
    }

    if (priceVal <= 0) {
        showModal('Error al agregar', 'No puedes agregar un cliente a la lista con esta cantidad');
        return;
    } 
    ...
    clientFunc.createNewClient(clientFromList.clientId, clientNameVal, priceVal, clientFromList.articles);
...
const sortLive = () => {
    ...
    for (let client of clientsArr) clientsHtmlFrgmnt.appendChild(clientFunc.createHtml(client.clientId, client.name, client.total, client.articles));
...
const saveLive = () => {
    ...
    if (clientsArr.length == 0 && !editOn) {
        showModal('Error al guardar', 'No hay ningun cliente que guardar en la tabla de clientes.');
        return;
    }
    
    /* Crear objeto del live e introducirlo en los lives existentes */
    const actualDate = new Date();
    const endTime = `${impFuncs.formatTime(actualDate.getHours())}:${impFuncs.formatTime(actualDate.getMinutes())}:${impFuncs.formatTime(actualDate.getSeconds())}`;
    const liveData = {
    ...
    showModal('Cargando..', 'Espere mientras se almacena la informacion del live en la base de datos.');
    ...
    else showModal('Live eliminado', 'Al sobreescribir el live sin ningun cliente se elimina de la base de datos');
    /* ------------------------------------------------------------- */

    /* Agregar el nuevo objeto a la base de datos */
    fbStore.collection('vendedoras').doc(userIdLS).set(fbLives)
        .then(() => {
            clearLive();
            open('index.html', '_self');
        }).catch(err => {
            showModal('Error al guardar', `Ocurrio un error al guardar la informacion en la base de datos, informe sobre este error: ${err}`)
        })
    /* ------------------------------------------- */
}
/* ---------------------------------------------------------- */
...
document.getElementById('btnPrint').addEventListener('click', () => {
    if (clientsArr.length == 0) {
        showModal('Error al imprimir', 'No hay ningun cliente que imprimir en la tabla de clientes.');
        return;
    }
    window.print();
})
...
document.addEventListener('keydown', (e) => {
    if (getShortcutsEnabled()) {
        if (e.altKey && e.code == 'KeyQ') document.getElementById('frmClientData').clientName.focus();
        if (e.altKey && e.code == 'KeyW') document.getElementById('frmClientData').price.focus();
